program benchmark_ampligluon_multiplet
  ...
  complex(benchmark_dp), allocatable :: basis(:)
  ...
  character(len=4096) :: argument, table_file
  character(len=64) :: backend_name, mhv_mode
  ...
  logical :: initialization_only, use_mhv_optimization

  argument_count = command_argument_count()
  if (argument_count < 5) call benchmark_fail( &
       'usage: benchmark_ampligluon_multiplet '// &
       'TABLE MHV_MODE TARGET_SECONDS BATCHES EVENT... '// &
       '[--initialization-only]')
  call get_command_argument(1, table_file)
  call get_command_argument(2, mhv_mode)
  select case (trim(mhv_mode))
  case ('optimized-mhv')
    use_mhv_optimization = .true.
    backend_name = 'AmpliGluonMultipletOptimizedMHV'
  case ('default-bg')
    use_mhv_optimization = .false.
    backend_name = 'AmpliGluonMultipletDefaultBG'
  case default
  ...
  call amplitude%initialize(events(1)%final_gluons, trim(table_file))
  ...
  do event_index = 1, number_of_events
    do configuration = 1, events(event_index)%number_of_helicities
      call amplitude%evaluate(events(event_index)%momenta, &
           events(event_index)%helicities(:, configuration), basis, &
  ...
  if (.not. use_mhv_optimization) then
    ...
    do event_index = 1, number_of_events
      do configuration = 1, events(event_index)%number_of_helicities
        ...
        do repetition_index = 1, &
          ...
          call amplitude%evaluate(events(event_index)%momenta, &
               events(event_index)%helicities(:, configuration), basis, &
               matrix_element, strong_coupling=events(event_index)%strong_coupling, &
               use_mhv_optimization=use_mhv_optimization)
...
end program benchmark_ampligluon_multiplet
